...
my(@column_names) = ('chatnet', 'nick', 'ident_nick', 'ident_passwd', 'nickserv', 'ident_enabled');
...
sub _split_args {
	return Text::ParseWords::parse_line('\s+',0,$_);
}
...
sub change_nick {
	...
	my(%conf) = $keepnick{lc $server->{chatnet}};
	if ($conf{ident_enabled}) {
			$server->send_message($conf{nickserv}, "$conf{ident_nick} $conf{ident_passwd}", 1);
	}
}
...
sub check_nick {
    ...
    for $net (keys %keepnick) {
		...
		next unless $server;
		next if lc $server->{nick} eq lc $keepnick{lc $net}->{nick};
	
		$getnick{$net} = $keepnick{lc $net}->{nick};
    }
...
sub load_nicks {
	my($csv) = Text::CSV_PP->new ( { binary => 1, empty_is_undef => 1 } )  # should set binary attribute.
	...
	my($entries) = $csv->getline_hr_all ( \*CONF );
    for my $e ( $entries ) {
		if ($e->{chatnet} && $e->{nick}) {
			$keepnick{lc $e->{chatnet}} = %e;
		    $count++;
		}
    }
    close CONF;
...
sub save_nicks {
	...
	open CONF, "> $file";
	####
	for my $row ( keys %keepnick ) {
		my(@ar);
		for my $col ( keys %keepnick{$row} ) {
			push @ar, %keepnick{$row}{$col};
		}
		my($status) = $csv->print (\*CONF, $arref);
		Irssi::printformat(MSGLEVEL_CLIENTCRAP, 'keepnick_crap', $status);
	}
	####
    close CONF;
...
sub sig_message_own_nick {
    ...
    } elsif (lc $oldnick eq lc $keepnick{lc $chatnet}->{nick} &&
...
sub cmd_keepnick {
    my(@params) = _split_args($_);
    my($server) = @_;
    my($chatnet, $nick, @opts);
	my($ident_nick, $ident_passwd, $ident_nickserv, $ident_enabled);
	$ident_enabled = 0; #false
    ...
    while (@params) {
		...
		} elsif ($param =~ /^-ident-nick/) {
			$ident_nick = shift @params;
			$ident_enabled = 1;
		} elsif ($param =~ /^-ident-passwd/) {
			$ident_passwd = shift @params;
			$ident_enabled = 1;
		} elsif ($param =~ /^-ident-nickserv/) {
            $ident_nickserv = shift @params;
			$ident_enabled = 1;
		} elsif ($param =~ /^-/) {
	...
	$ident_nick ||= $nick;
	$ident_passwd ||= '';
	$ident_nickserv ||= 'NICKSERV';

    # check that we really have a chatnet
    unless ($chatnet) {
		Irssi::printformat(MSGLEVEL_CLIENTCRAP, 'keepnick_crap', "Unable to find a chatnet");
		return;
    }
	
    if ($inactive{lc $chatnet}) {
		delete $inactive{lc $chatnet};
		Irssi::printformat(MSGLEVEL_CLIENTCRAP, 'keepnick_unhold', $nick, $chatnet);
    }

    Irssi::printformat(MSGLEVEL_CLIENTCRAP, 'keepnick_add', $nick, $chatnet);
	
	$keepnick{lc $chatnet} = (chatnet=>lc $chatnet,nick=>$nick, ident_nick=>$ident_nick, ident_passwd=>$ident_passwd, nickserv=>$ident_nickserv, ident_enabled=>$ident_enabled);
