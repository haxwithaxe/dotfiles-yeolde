...
sub _calculate_items {
	...
	$display = "$display%n";
	my $num_ent_padding = $numPad - length $number;
	my $num_ent = (' 'x($num_ent_padding < 0 ? 0 : $num_ent_padding)) . $number;
	my $key_length = length $keymap{$number};
	my $key_ent_padding = $keyPad - $key_length;
	my $key_ent = exists $keymap{$number} ? ((' 'x($key_ent_padding < 0 ? 0 : $key_ent_padding)) . $keymap{$number}) : ' 'x$keyPad;
